...
export function installFileDrop(api: TerminalAPI, target: () => UploadTarget | string) {
  ...
  const finish = (report: UploadReport) => {
    ...
    title.textContent = report.cancelled ? 'Upload cancelled' : failed ? 'Upload finished with errors' : 'Upload complete';
    ...
    for (const item of report.items) {
      ...
      li.textContent = `${item.name} — ${item.status}${item.detail ? ': ' + item.detail : ''}`; results.append(li);
    }
  ...
  api.onFileDrop(async event => {
    ...
    if (event.kind === 'enter') {
      ...
      heading.textContent = typeof selected === 'string' ? 'Cannot upload here' : 'Drop files or folders to upload';
    ...
    try {
      const report = await api.uploadDroppedFiles(selected.id, selected.win, token);
  ...
  api.onUploadProgress(progress => {
    ...
    title.textContent = `Uploading ${Math.min(progress.completed + 1, progress.count)} of ${progress.count}`;
  ...
  return { reset() { if (token) void api.cancelFileUpload(token).catch(() => {}); token = null; overlay.hidden = true; panel.hidden = true; } };
}
